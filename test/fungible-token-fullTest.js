...
describe('Fungible Token Full Contract', () => {
  ...
  it('Fungible Token Contract: Mint Tokens', async () => {
    ...
    assert.equal(
      Crypto.addressFromDecimal(mint.result.log[0].topics[1]),
      wallets[0].publicKey,
    );
  ...
  it('Fungible Token Contract: Burn Tokens', async () => {
    ...
    assert.equal(
      Crypto.addressFromDecimal(burn.result.log[0].topics[1]),
      wallets[0].publicKey,
    );
  ...
  it('Fungible Token Contract: Create Allowance', async () => {
    const create_allowance = await contract.methods.create_allowance(
      wallets[1].publicKey,
      10,
    );
    assert.equal(topicHashFromResult(create_allowance), hashTopic('Allowance'));
    assert.equal(
      Crypto.addressFromDecimal(create_allowance.result.log[0].topics[1]),
      wallets[0].publicKey,
    );
    assert.equal(
      Crypto.addressFromDecimal(create_allowance.result.log[0].topics[2]),
      wallets[1].publicKey,
    );
  ...
  it('Fungible Token Contract: Get Allowance', async () => {
    ...
    const allowance_for_caller = await contract.methods.allowance_for_caller(
      wallets[0].publicKey,
  ...
  it('Fungible Token Contract: Increase Allowance', async () => {
    ...
    await contract.methods
      .change_allowance(wallets[1].publicKey, 10)
      .catch((e) => e);
  ...
  it('Fungible Token Contract: Transfer Allowance', async () => {
    ...
    await contract.methods.transfer_allowance(
      wallets[0].publicKey,
      wallets[1].publicKey,
      5,
      { onAccount: wallets[1].publicKey },
    ...
    assert.deepEqual(balances.decodedResult, [
      [wallets[0].publicKey, 5],
      [wallets[1].publicKey, 5],
    ]);
  });

  it('Fungible Token Contract: Transfer Allowance (should fail)', async () => {
    const allowanceFailBalance = await contract.methods
  ...
  it('Fungible Token Contract: Reset Allowance', async () => {
    ...
    await contract.methods
      .reset_allowance(wallets[1].publicKey)
      .catch((e) => e);
