...
const getFilesystem = (contractSource) => {
  console.log(
    `Creating filesystem by checking includes for: ${contractSource}`,
  );
  ...
  const rgx = /^include\s+\"([\d\w\/\.\-\_]+)\"/gim;
  const rgxIncludePath = /"([\d\w\/\.\-\_]+)\"/gim;
  const rgxMainPath = /.*\//g;
  ...
  for (let i = 0; i < rootIncludes.length; i++) {
    ...
    const includePath = path.resolve(
      `${contractPath[0]}/${includeRelativePath[1]}`,
    );
    try {
