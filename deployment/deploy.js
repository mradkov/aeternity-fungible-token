...
const CONTRACT_SOURCE = './contracts/fungible-token.aes';
...
const deploy = async (secretKey, network, compiler) => {
  ...
  const client = await Universal({
    nodes: [
      {
        name: NETWORK_NAME,
        instance: await Node({ url: NETWORKS[NETWORK_NAME].nodeUrl }),
      },
    ],
  ...
  const contract_content = contractUtils.getContractContent(CONTRACT_SOURCE);
